...
import { FaRegHeart, FaSortAmountUpAlt } from "react-icons/fa";
...
function Home() {
  ...
  return (
    <>
      ...
      {/* Set The Lower header section  Or use your Own  header make it a seperate component */}
      <Header />
      <BannerSection />

      <div className={classes.heading}>
        <h4>HOW WE HELP OUR CUSTOMERS</h4>
      </div>
      <div className={classes.services}>
        {param.map((ele, index) => (
          <div className={classes.types}>
            <div className={classes.logo}>{ele.name}</div>
            <h5>{ele.heading}</h5>
            <p>{ele.lines}</p>
        ...
        {Array.from({ length: 3 }).map((ele, index) => (
          <div className={classes.card}>
            <div className={classes.card_top}>
              ...
              <div className={classes.heart_tag}>
                <FaRegHeart size={25} />
              </div>
            </div>
            <div className={classes.card_bottom}>
              <h6>Luxury Apartment</h6>
              Experience unparalleled elegance and comfort in our luxury
              apartments, where sophisticated design meets top-tier amenities.
              <span>
                <b>Address:</b>DHA Phase 6,Karachi
              </span>
              <div className={classes.icon}>
                <div className={classes.sub_icon}>
                  <IoMdBed size={25} />4 Rooms
                </div>
                <div className={classes.sub_icon}>
                  <LiaBathSolid size={25} />2 Washroom
                </div>
              </div>
            </div>
          </div>
