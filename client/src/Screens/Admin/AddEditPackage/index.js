...
import { Get, Patch, Post } from "../../../AxiosFunction/AxiosFunction";
...
function AddEditPackage() {
  ...
  const handleGetPackage = async () => {
    const apiUrl = BaseUrl(`admin/packages/${id}`);
    setLoading("main");

    const response = await Get(apiUrl, token);
    if (response?.data) {
      const prevPackage = response?.data?.package;
      setTitle(prevPackage?.title);
      setPackageId(prevPackage?._id);
      setDescription(prevPackage?.description);
      setPrice(prevPackage?.price);
      setIncludePermission(prevPackage?.permission ?? []);
      setRecurringType(
        recurringTypeOptions?.find(
          (e) => e?.value == prevPackage?.recurringType
        ) || null
      );
      setType(typeOptions?.find((e) => e?.value == prevPackage?.type) || null);
    }
    setLoading(false);
  };
        ...
        {loading == "main" ? (
          ...
          <>
            ...
            <div className={classes.buttonMain}>
              <Button
                label={loading == "editing" ? "Submitting..." : "Submit"}
                disabled={loading}
